...
add_custom_command(OUTPUT ${DEFCONFIG_OVERRIDE_PATH}
	COMMAND ${CMAKE_COMMAND} -E copy_if_different "${PROJECT_SOURCE_DIR}/src/lkl/override/defconfig" "${DEFCONFIG_OVERRIDE_PATH}"
	MAIN_DEPENDENCY "${PROJECT_SOURCE_DIR}/src/lkl/override/defconfig"
...
add_custom_command(
	...
	COMMAND ${CMAKE_COMMAND} -E copy "${PROJECT_SOURCE_DIR}/${LKL_SUBDIRECTORY}/net/Makefile" "${PROJECT_SOURCE_DIR}/${LKL_SUBDIRECTORY}/net/Kconfig" "${CMAKE_CURRENT_BINARY_DIR}/lkl/net"
	COMMAND ${CMAKE_COMMAND} -E remove_directory "${CMAKE_CURRENT_BINARY_DIR}/lkl/net/wireguard"
	COMMAND "${WIREGUARD_SOURCE_DIR}/contrib/kernel-tree/create-patch.sh" | patch --forward -p1
	COMMAND ${CMAKE_COMMAND} -E touch "${CMAKE_CURRENT_BINARY_DIR}/wireguard-patches-applied.stamp"
	WORKING_DIRECTORY "${CMAKE_CURRENT_BINARY_DIR}/lkl" 
	)
...
set(LKL_HEADERS_FILE "${CMAKE_CURRENT_BINARY_DIR}/lkl-headers.list")
...
set(LKL_HEADER_PATH "${CMAKE_CURRENT_BINARY_DIR}/lkl-headers")
add_custom_command(OUTPUT "${LKL_LIB_PATH}"
	DEPENDS lkl-source-setup
	COMMAND make -C "${CMAKE_CURRENT_BINARY_DIR}/${LKL_SUBDIRECTORY}/tools/lkl" -j ${NUMBER_OF_CORES} V=1 "EXTRA_CFLAGS=${LKL_EXTRA_CFLAGS}" ${LKL_EXTRA_OPTIONS} "${CMAKE_CURRENT_BINARY_DIR}/${LKL_LIB_PATH}"
	COMMAND ${CMAKE_COMMAND} -E env "DESTDIR=${LKL_HEADER_PATH}" make -C "${CMAKE_CURRENT_BINARY_DIR}/${LKL_SUBDIRECTORY}/tools/lkl/" -j ${NUMBER_OF_CORES} V=1 "PREFIX=\"\"" headers_install
	COMMAND make -C "${CMAKE_CURRENT_BINARY_DIR}/${LKL_SUBDIRECTORY}" ARCH=lkl "INSTALL_HDR_PATH=${LKL_HEADER_PATH}" -j ${NUMBER_OF_CORES} V=1 "PREFIX=\"\"" headers_install
