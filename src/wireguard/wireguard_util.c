...
bool wgu_parse_ip(wg_allowedip* allowedip, const char* value)
{
    ...
    {
        sgxlkl_error("Unable to parse IP address: `%s'\n", value);
        return false;
    }
    return true;
}
    ...
    const char* value)
{
    ...
    while ((mask = strsep(&sep, ",")))
    {
        ...
        if (mask)
        {
            ...
            if (*end || (cidr > 32 && new_allowedip->family == AF_INET) ||
...
err:
    free(new_allowedip);
    free(mutable);
    sgxlkl_error("AllowedIP is not in the correct format: `%s'\n", saved_entry);
    free(saved_entry);
    return false;
}
...
bool wgu_parse_endpoint(struct sockaddr* endpoint, const char* value)
{
    ...
    {
        free(mutable);
        sgxlkl_error("Unable to parse empty endpoint\n");
        return false;
    }
    if (mutable[0] == '[')
    {
        ...
        *end++ = '\0';
        ...
        {
            free(mutable);
            sgxlkl_error("Unable to find port of endpoint: `%s'\n", value);
            return false;
        }
    }
    else
    ...
    for (unsigned int timeout = 1000000;;)
    {
         ...
         *
         * This is according to https://sourceware.org/glibc/wiki/NameResolver
         * which states: "From the perspective of the application that calls
         * getaddrinfo() it perhaps doesn't matter that much since EAI_FAIL,
         * EAI_NONAME and EAI_NODATA are all permanent failure codes and the
         * causes are all permanent failures in the sense that there is no point
         * in retrying later."
         *
         * So this is what we do, except FreeBSD removed EAI_NODATA some time
         * ago, so that's conditional.
         */
        ...
        sgxlkl_warn(
            "%s: `%s'. Trying again in %.2f seconds...\n",
            ret == EAI_SYSTEM ? strerror(errno) : gai_strerror(ret),
            value,
            timeout / 1000000.0);
        usleep(timeout);
    ...
    else
    {
        ...
        free(mutable);
        sgxlkl_error("Neither IPv4 nor IPv6 address found: `%s'\n", value);
        return false;
    }
    freeaddrinfo(resolved);
    free(mutable);
    ...
    bool set_device)
{
    ...
    for (int i = 0; i < num_peers; i++)
    {
        ...
        {
            sgxlkl_warn("Unable to add wireguard peer due to missing allowed "
                        "ips configuration.\n");
            continue;
        }
        ...
        if (!wgu_parse_allowedips(
                &new_peers[i], &pallowedip, peer_cfg.allowed_ips))
        {
            ret = -EINVAL;
            goto err;
        }
