...
static void parse_epoll_event_flags(char *buf, size_t buf_len, struct epoll_event *evt) {
    size_t written = 0;
    ...
    if (evt->events & EPOLLOUT) {
        written = snprintf(buf, buf_len - written, "%sEPOLLOUT", written ? "|" : "");
        buf += written;
    }
    if (evt->events & EPOLLRDHUP) {
        written = snprintf(buf, buf_len - written, "%sEPOLLRDHUP", written ? "|" : "");
        buf += written;
    }
    if (evt->events & EPOLLPRI) {
        written = snprintf(buf, buf_len - written, "%sEPOLLPRI", written ? "|" : "");
        buf += written;
    }
    if (evt->events & EPOLLERR) {
        written = snprintf(buf, buf_len - written, "%sEPOLLERR", written ? "|" : "");
        buf += written;
    }
    if (evt->events & EPOLLHUP) {
        written = snprintf(buf, buf_len - written, "%sEPOLLHUP", written ? "|" : "");
        buf += written;
    }
    if (evt->events & EPOLLET) {
        written = snprintf(buf, buf_len - written, "%sEPOLLET", written ? "|" : "");
        buf += written;
    }
    if (evt->events & EPOLLONESHOT) {
        written = snprintf(buf, buf_len - written, "%sEPOLLONESHOT", written ? "|" : "");
        buf += written;
    }
    if (evt->events & EPOLLWAKEUP) {
        written = snprintf(buf, buf_len - written, "%sEPOLLWAKEUP", written ? "|" : "");
        buf += written;
    }
...
void log_sgxlkl_syscall(int type, long n, long res, int params_len, ...) {
    ...
    if (n == SYS_open || n == SYS_lstat) {
        ...
        SGXLKL_TRACE_SYSCALL(type, "[tid=%-3d] %s\t%ld\t(%d, %d, %d, %p {%s}) = %ld %s\n", tid, name, n,
            (int) params[0], (int) params[1], (int) params[2], evt, event_flags, res, errmsg);
    } else if (n == SYS_epoll_wait || n == SYS_epoll_pwait) {
        char event_flags[EPOLL_EVENT_FLAG_BUFFER_LEN];
        struct epoll_event *evt = (struct epoll_event *) params[1];
        parse_epoll_event_flags(event_flags, EPOLL_EVENT_FLAG_BUFFER_LEN, evt);
        SGXLKL_TRACE_SYSCALL(type, "[tid=%-3d] %s\t%ld\t(%d, %p {%s}, %d, %d, %ld) = %ld %s\n", tid, name, n,
            (int) params[0], evt, event_flags, (int) params[2], (int) params[3], params[4], res, errmsg);
    } else {
...
#else
inline void log_sgxlkl_syscall(int type, long n, long res, int params_len, ...) { /* empty */ }
#endif /* DEBUG */
