...
/*
 * Copyright (C) 2015-2018 Jason A. Donenfeld <Jason@zx2c4.com>. All Rights
 * Reserved. Copyright (C) 2008-2012 Pablo Neira Ayuso <pablo@netfilter.org>.
 */
...
#define mnl_attr_for_each(attr, nlh, offset)                                  \
    for ((attr) = mnl_nlmsg_get_payload_offset((nlh), (offset)); mnl_attr_ok( \
             (attr), (char*)mnl_nlmsg_get_payload_tail(nlh) - (char*)(attr)); \
         (attr) = mnl_attr_next(attr))

#define mnl_attr_for_each_nested(attr, nest)                      \
    for ((attr) = mnl_attr_get_payload(nest); mnl_attr_ok(        \
             (attr),                                              \
             (char*)mnl_attr_get_payload(nest) +                  \
...
static void* mnl_nlmsg_get_payload_offset(
...
static bool mnl_attr_ok(const struct nlattr* attr, int len)
{
    return len >= (int)sizeof(struct nlattr) &&
           attr->nla_len >= sizeof(struct nlattr) && (int)attr->nla_len <= len;
}
    ...
    void* data)
{
    ...
    mnl_attr_for_each(
        attr,
        nlh,
        offset) if ((ret = cb(attr, data)) <= MNL_CB_STOP) return ret;
    return ret;
}

static int mnl_attr_parse_nested(
...
static bool mnl_attr_put_u8_check(
    struct nlmsghdr* nlh,
    size_t buflen,
    uint16_t type,
    uint8_t data)
...
static bool mnl_attr_put_u16_check(
    struct nlmsghdr* nlh,
    size_t buflen,
    uint16_t type,
    uint16_t data)
...
static bool mnl_attr_put_u32_check(
    struct nlmsghdr* nlh,
    size_t buflen,
    uint16_t type,
    uint32_t data)
...
static struct nlattr* mnl_attr_nest_start_check(
    struct nlmsghdr* nlh,
    size_t buflen,
    uint16_t type)
{
...
static ssize_t mnl_socket_sendto(
...
static ssize_t mnl_socket_recvfrom(
    const struct mnl_socket* nl,
    void* buf,
    size_t bufsiz)
{
...
static struct nlmsghdr* mnlg_msg_prepare(
    struct mnlg_socket* nlg,
    uint8_t cmd,
    uint16_t flags)
{
...
static int mnlg_socket_recv_run(
    struct mnlg_socket* nlg,
    mnl_cb_t data_cb,
    void* data)
{
...
static struct mnlg_socket* mnlg_socket_open(
...
another:
    if ((len = mnl_socket_recvfrom(nl, rtnl_buffer, MNL_SOCKET_BUFFER_SIZE)) <
        0)
    {
        ret = -errno;
        goto cleanup;
    ...
    if ((len = mnl_cb_run(
             rtnl_buffer, len, seq, portid, read_devices_cb, buffer)) < 0)
    {
    ...
    for (peer = peer ? peer : dev->first_peer; peer; peer = peer->next_peer)
    {
        ...
        if (flags)
        {
            if (!mnl_attr_put_u32_check(
                    nlh, MNL_SOCKET_BUFFER_SIZE, WGPEER_A_FLAGS, flags))
                goto toobig_peers;
        }
        if (peer->first_allowedip)
        {
            ...
            allowedips_nest = mnl_attr_nest_start_check(
                nlh, MNL_SOCKET_BUFFER_SIZE, WGPEER_A_ALLOWEDIPS);
            if (!allowedips_nest)
                goto toobig_allowedips;
            for (; allowedip; allowedip = allowedip->next_allowedip)
            {
                allowedip_nest =
                    mnl_attr_nest_start_check(nlh, MNL_SOCKET_BUFFER_SIZE, 0);
                if (!allowedip_nest)
                    goto toobig_allowedips;
                if (!mnl_attr_put_u16_check(
                        nlh,
    ...
    switch (mnl_attr_get_type(attr))
    {
        ...
        case WGPEER_A_ENDPOINT:
        {
            ...
            if (addr->sa_family == AF_INET &&
                ...
                memcpy(
                    &peer->endpoint.addr4, addr, sizeof(peer->endpoint.addr4));
            else if (
                addr->sa_family == AF_INET6 &&
                mnl_attr_get_payload_len(attr) == sizeof(peer->endpoint.addr6))
                memcpy(
                    &peer->endpoint.addr6, addr, sizeof(peer->endpoint.addr6));
            break;
        }
...
static void coalesce_peers(wg_device* device)
{
    ...
    {
        if (memcmp(
                peer->public_key, peer->next_peer->public_key, sizeof(wg_key)))
        {
            peer = peer->next_peer;
            continue;
        }
...
try_again:
    *device = calloc(1, sizeof(wg_device));
    ...
    nlh = mnlg_msg_prepare(
        nlg, WG_CMD_GET_DEVICE, NLM_F_REQUEST | NLM_F_ACK | NLM_F_DUMP);
...
void wg_free_device(wg_device* dev)
{
    ...
    {
        for (allowedip = peer->first_allowedip,
...
{
    const uint8_t input[] = {(src[0] >> 2) & 63,
                             ((src[0] << 4) | (src[1] >> 4)) & 63,
                             ((src[1] << 2) | (src[2] >> 6)) & 63,
                             src[2] & 63};
    unsigned int i;
...
void wg_key_to_base64(wg_key_b64_string base64, const wg_key key)
{
    ...
    encode_base64(
        &base64[i * 4], (const uint8_t[]){key[i * 3 + 0], key[i * 3 + 1], 0});
